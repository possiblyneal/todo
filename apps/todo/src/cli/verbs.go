...
func repeatTask(s *store.Store, args []string, stdout, stderr io.Writer) int {
	...
	// write.Mark's shape rather than a second copy of it here.
	for _, asked := range []struct {
		mark string
		date string
	}{{"tick", *tick}, {"skip", *skip}, {"detach", *detach}} {
		if asked.date == "" {
			continue
		}
		on, err := time.ParseInLocation(time.DateOnly, asked.date, time.UTC)
		if err != nil {
			fmt.Fprintf(stderr, "todo repeat: cannot read %q as a date: want 2006-01-02\n", asked.date)
			return 2
		}
		act, _ := write.Mark(asked.mark)
	...
	case rule != "":
		_, err := write.Repeat(s, who, id, rule)
		return refuse(stderr, "repeat", err)
	}
