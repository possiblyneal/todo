...
- **Completing refuses over an open child; reopening one and adding one cascade instead.** Completing is the direction a person drives, so it is the direction that says no. The other two cannot refuse without stranding the person, so each reopens every completed Task above it. Both walks are a `WITH RECURSIVE ancestry` inside one `UPDATE`: `recursive_triggers` is off by default, so a trigger firing a trigger reaches exactly one level and silently stops.
- **A Task out of sight takes its subtree with it.** Every narrowing a read applies — deleted, completed, snoozed and the List — goes inside both arms of the `depth_first` walk, not in the outer `WHERE`, so a subtree is reachable only through a visible root. Filtering per row instead leaves children on the screen under a parent that is gone. The rule binds a surface that narrows in Go too: the TUI's Tag filter drops the run of deeper rows under a Task the chosen Tags pass over.
...
- **What a read worked out about a Task is `Task.Marks`.** Overdue, done, deleted and snoozed are decided once in the store and said in the same words everywhere, so a Task cannot read as snoozed from a keyboard and plain on a screen. A surface decides only how to draw them.
