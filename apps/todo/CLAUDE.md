...
- **A Task out of sight takes its subtree with it.** Deleted, completed and snoozed are filtered inside both arms of the `depth_first` walk, not in the outer `WHERE`, so a subtree is reachable only through a visible root. Filtering per row instead leaves children on the screen under a parent that is gone.
- **`store.Sorts` is the one list of sorts there is.** The error a bad `-sort` gets and the flag help a person reads both come from it, so neither can name a set the store does not have.
